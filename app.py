...
app = Flask(__name__)
app.secret_key = '1234' # requrie for sessions
...
def create():
    if request.method == 'POST':
        ...
        if any (record ['id'] == new_id for record in data ):
            ...
            #flash(f'Record Create Successfully', 'sucess')
            return render_template('index.html', data = data)
...
def delete(id):
    ...
    if len(data) == len(new_data):
        flash(f"Error : no record found ID {id}", 'danger')
    else:
        save_data(new_data)
...
def edit(id):
    ...
    if record is None:
        flash("Error: no record found with ID {id}", 'warning')
        return redirect(url_for('index'))
    if request.method == 'POST':
